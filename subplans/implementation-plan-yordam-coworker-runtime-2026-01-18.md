...
## 11) Open Questions
Locked decisions:
- Q1 (medium): Yes. Require per-path locking before enabling multi-worker; default stays single worker.
- Q2 (low): No. Previews remain bundle files; SQLite stores metadata + pointers only.
- Q3 (low): Yes. Include minimal `launchd print-plist` in v1 (optional/manual).
